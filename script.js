document.addEventListener('DOMContentLoaded', function() {
    ...
    modules.forEach(module => {
        ...
        const posterMeta = module.querySelector('.poster-meta'); // Находим текст со временем на постере
        const playerWrapper = module.querySelector('.player-wrapper'); // Для фикса фуллскрина
        ...
        // ==============================================================
        // ФИКС ДЛЯ ПОЛНОЭКРАННОГО РЕЖИМА НА МОБИЛЬНЫХ (iOS/Android)
        // ==============================================================
        player.on('enterfullscreen', () => {
            // При входе в фуллскрин жестко отключаем все CSS-трансформы и фильтры,
            // которые запирают видео внутри блока на мобильных устройствах
            if (module) {
        ...
        player.on('exitfullscreen', () => {
            // При выходе из фуллскрина убираем инлайн-стили, чтобы 
            // вернулись красивые эффекты из CSS (блюр, закругления)
            if (module) {
        ...
        // Когда браузер загрузит метаданные видео (включая его длину)
        player.on('loadedmetadata', () => {
            if (posterMeta && player.duration) {
                const totalTime = formatTime(player.duration);
                // Заменяем текст внутри <p class="poster-meta">, сохраняя иконку часов
        ...
        if (customPoster) {
            // Клик по самому постеру
            customPoster.addEventListener('click', () => {
                player.play(); 
            });

            // Скрываем постер при начале воспроизведения
            player.on('play', () => {
    ...
    // 2. ЛОГИКА ПЕРЕКЛЮЧЕНИЯ МЕНЮ СЛЕВА (Сброс видео и постеров)
